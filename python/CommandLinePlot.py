...
def main(argv):
    ...
    try:
        ...
        opts, args = getopt.getopt(argv, "h", ["xdata=", "ydata=","xlabel=","ylabel=", "legend=", "saveplot=", "title="])
...
def plot(x_data, y_data,x_label, y_label, legend, title, save_plot_file):
    ...
    for i_index in range( len(y_data_sets)):
        ...
        line, = ax1.plot( x_coordinates, y_coordinates, color=colors[ i_index % len(colors)  ], lw=standard_line_width, label=legend_name)
